...
function buildPositionAdvice(action, riskLevel, ps, report) {
  ...
  // C类（已清仓等接回）：SELL方向意味着下行风险未除，继续空仓观望
  if (posState === "C") {
    if (action === "SELL_WAVE") return {
      label: "继续空仓观望", color: "#ff4757",
      bg: "rgba(255,71,87,0.06)", border: "rgba(255,71,87,0.2)",
      first: "下行风险未除，暂不接回",
      followup: "等买入信号确认后再分批接回",
      stopNote: "",
      basis: sup ? `支撑位参考 ${sup} 元，跌穿则信号进一步推迟` : "",
    };
    if (action === "WATCH_SELL") return {
      label: "暂缓接回", color: "#ffa502",
      bg: "rgba(255,165,2,0.06)", border: "rgba(255,165,2,0.2)",
      first: "潜在下行风险仍在，观望为主",
      followup: "风险信号消退或出现明确止跌后再考虑接回",
      stopNote: "",
      basis: sup ? `接回参考支撑位 ${sup} 元` : "",
    };
  }
...
export default function App() {
  ...
  const handleFile = useCallback((text, name) => {
    ...
    try {
      ...
      // Python/pandas 生成的 JSON 可能含 NaN（对象值或数组裸值），不符合 JSON 规范，统一替换为 null
      const clean = stripped.replace(/\bNaN\b/g, "null");
          ...
          return (
            <div style={{ animation:"fadeUp .4s ease" }}>
              ...
              <div style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.05)", borderRadius:16, padding:"20px 24px", marginBottom:14 }}>
                ...
                <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                  ...
                  {(() => {
                    ...
                    // order_price来源：有本地数据时严格只用Python值，防止AI幻觉价格被📊图标背书
                    // Python是HOLD但AI升级为BUY/SELL时，ps.order_price为null，直接走ATR容错
                    const rawOp = ps ? ps.order_price : ai?.order_price;
              ...
              {(() => {
                ...
                const rawOp2 = ps ? ps.order_price : ai?.order_price;
